...
<span><a href="{{ route('import.create', ['double-entry', 'chart_of_accounts']) }}" class="btn btn-white btn-sm header-button-top"><span class="fa fa-download"></span> &nbsp;{{ trans('import.import') }}</a></span>
@endpermission
    ...
    <div class="card">
        ...
        <div class="table-responsive">
            <table class="table table-flush table-hover" id="tbl-taxes">
                ...
                <tbody>
                    @foreach($class->types->sortBy('id') as $type)
                        @foreach($class->accounts->where('type_id', $type->id)->sortBy('name') as $item)
                            <tr class="row align-items-center border-top-1">
                                <td class="col-md-1 border-0">{{ $item->code }}</td>
                                <td class="col-md-5 border-0 long-texts"><a href="{{ route('chart-of-accounts.edit', $item->id) }}">{{ trans($item->name) }}</a></td>
                                ...
                                <td class="col-md-2 text-center border-0">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                            ...
                                            <div class="dropdown-divider"></div>
                                            {!! Form::deleteLink($item, 'chart-of-accounts.destroy', 'double-entry::general.chart_of_accounts') !!}
                                            @endpermission
                                        </div>
                                    </div>
                                </td>
